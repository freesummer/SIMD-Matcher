...
#include "idx/utils/include/idx/utils/RandomRangeGenerator.hpp"
...
/*
 * New function: creatDataSet_pair()
...
std::vector<std::pair<uint64_t, uint64_t>> creatDataSet_pair(CommandParser const & params, std::string const & typeParamName) {
  std::string dataSetType = params.expect<std::string>(typeParamName);
  std::vector<std::pair<uint64_t, uint64_t>> values;

  if(dataSetType == "file") {
    std::string inputFile = params.expectExistingFile("input");
    values = idx::utils::readDatFile_pair(inputFile, params.get("size", 0)); // set the default parameter "size = 0"
  }
