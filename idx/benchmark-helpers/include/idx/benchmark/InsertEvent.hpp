...
/*
 * pair<PairValueType, PairMaskType> value
 * PairValueType: value field
 * PairMaskType: mask field ====> indicate the wildcard positions
 * rule(value, mask)
 */
template<typename PairValueType, typename PairMaskType, template <typename> typename KeyExtractor> InsertEvent<std::pair<PairValueType, PairMaskType>, KeyExtractor> InsertEvent<std::pair<PairValueType, PairMaskType>, KeyExtractor>::read(std::istream const & input) {
  std::pair<PairValueType, PairMaskType> value;
