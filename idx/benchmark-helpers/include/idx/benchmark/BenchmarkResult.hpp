...
private:
  ...
  IndexStatistics memoryInformations_left;
  IndexStatistics memoryInformations_right;
	std::map<std::string, OperationResult> results;
  std::map<std::string, OperationResult> results_left;
  std::map<std::string, OperationResult> results_right;
...
public:
  ...
  std::map<std::string, OperationResult> & getResults_left() {
    return results_left;
  }

  std::map<std::string, OperationResult> getResults_right() const {
  ...
  void add_left(const std::string &operationName, const OperationResult &result) {
    results_left[operationName] = result;
  }

  void add_right(const std::string &operationName, const OperationResult &result) {
    results_right[operationName] = result;
  }

	void setIndexStatistics(IndexStatistics const &memoryConsumption) {
    #ifdef PING_DEBUG14
    std::cout << "setIndexStatistics function:" << std::endl;
#endif
		memoryInformations = memoryConsumption;
	}

  void setIndexStatistics_left(IndexStatistics const &memoryConsumption) {
    #ifdef PING_DEBUG14
    std::cout << "setIndexStatistics_left function:" << std::endl;
#endif
    memoryInformations_left = memoryConsumption;
  }

  void setIndexStatistics_right(IndexStatistics const &memoryConsumption) {
    #ifdef PING_DEBUG14
    std::cout << "setIndexStatistics_right function:" << std::endl;
#endif
    memoryInformations_right = memoryConsumption;
  }
...
//    memoryInformations_left.writeYAML(output, depth + 1);
//    memoryInformations_right.writeYAML(output, depth + 1);
