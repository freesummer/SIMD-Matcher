...
#include <memory>
#include <cstdlib>
#include <idx/utils/include/idx/utils/DataSetGenerators.hpp>
#include <idx/utils/include/idx/utils/CommandParser.hpp>
#include <idx/utils/include/idx/utils/DataSetModifiers.hpp>
#include <idx/utils/include/idx/utils/8ByteDatFileIO.hpp>
...
//	std::shared_ptr<std::vector<uint64_t>> mInsertKeys;
  std::shared_ptr<std::vector<std::pair<uint64_t, uint64_t>>> mInsertKeys;
...
//    createLookupKeys(params); // create lookup keys from insertkeys
//	}

  /*
   * New function: BenchmarkConfiguration_pair()
   * changed type for mInsertKeys
   *
   */

  BenchmarkConfiguration(idx::utils::CommandParser const & params)
      : mInsertKeys(new std::vector<std::pair<uint64_t, uint64_t>>)
      , mLookupKeys(new std::vector<uint64_t>)
      , mCommandParser(params)
      , mRawArguments(params.getRawArguments())
      , mInsertOnly(params.get<bool>("insertOnly", false))
      , mNumberThreads(params.get<size_t>("threads", 1))
  {
    *mInsertKeys = idx::utils::creatDataSet_pair(params, "insert");
    applyInputModifier(params);
    createLookupKeys(params); // create lookup keys from insertkeys
  }
...
//	}
  ...
  std::vector<std::pair<uint64_t, uint64_t>> & getInsertValues() {
    return *mInsertKeys;
  }
...
private:
  ...
  //  use insertKeys generate lookup keys
...
//        idx::utils::sortData(*mLookupKeys); //  std::sort()
