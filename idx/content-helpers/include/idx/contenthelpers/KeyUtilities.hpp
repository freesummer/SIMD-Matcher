...
template<typename KeyType> inline auto toBigEndianByteOrder(KeyType const & key) {
  std::cout << "===toBigEndianByteOrder=== key = " << key << std::endl;
	return key;
};

template<> __attribute__((always_inline)) inline auto toBigEndianByteOrder<uint64_t>(uint64_t const & key) {
  #ifdef PING_DEBUG3
  printf("toBigEndianByteOrder<uint64_t> function:-------uint64_t \n");
  std::cout << "bswap_64(key) = " << std::bitset<64>(__bswap_64(key)) << std::endl;
#endif
...
template<typename KeyType> __attribute__((always_inline)) inline uint8_t const * interpretAsByteArray(KeyType const & key) {
  //std::cout << "==interpretAsByteArray== result is " << reinterpret_cast<uint8_t const *>(&key) << std::endl;
	return reinterpret_cast<uint8_t const *>(&key);
}

/*
 * added a function: non-const version
 *
 */
template<typename KeyType> __attribute__((always_inline)) inline uint8_t * interpretAsByteArray(KeyType & key) {
  //std::cout << "==interpretAsByteArray== result is " << reinterpret_cast<uint8_t const *>(&key) << std::endl;
