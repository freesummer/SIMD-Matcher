...
#include "hot/commons/include/hot/commons/MultiMaskPartialKeyMappingInterface.hpp"
...
template<unsigned int numberExtractionMasks> inline __attribute__((always_inline)) uint32_t MultiMaskPartialKeyMapping<numberExtractionMasks>::extractMask(uint8_t const * keyBytes, uint64_t mask) const {
  return extractMaskForMappedInput(mapInput(keyBytes));
}
...
template<unsigned int numberExtractionMasks> template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<numberExtractionMasks>::getPrefixBitsMask(DiscriminativeBit const &significantKeyInformation) const {
	//initialized to zero
  #ifdef PING_DEBUG3
  printf("------MultiMaskPartialKeyMapping----------getPrefixBitsMask function() ----begin\n");
  #endif
  ExtractionDataArray prefixBits = zeroInitializedArray();
  #ifdef PING_DEBUG3
  printf("numberExtractionMasks = %d\n", numberExtractionMasks);
   #endif
	uint8_t subtreeMissmatchingByte = static_cast<uint8_t >(UINT8_MAX) << (8 - significantKeyInformation.mByteRelativeBitIndex);
   #ifdef PING_DEBUG3
  printf("subtreeMissmatchingByte = %d, mNumberExtractionBytes = %d\n", subtreeMissmatchingByte, mNumberExtractionBytes);
 #endif
  ...
  #ifdef PING_DEBUG
  printf("prefixBits.size = %lu\n", prefixBits);
  for (int i = 0; i < prefixBits.size(); i++) {
    std::cout << "i = " << i << "value = " << std::bitset<64>(prefixBits[i]) << std::endl;
  }
  #endif
...
#ifdef PING_DEBUG3
      std::cout << "mapInput function: " << std::endl;
#endif
...
#ifdef PING_DEBUG3
      std::cout << "mNumberExtractionBytes =  "  << mNumberExtractionBytes << std::endl;
#endif
	for(int i=0; i < mNumberExtractionBytes; ++i) {
		mappedInputBytes[i] = keyBytes[positions[i]];
#ifdef PING_DEBUG3
      std::cout << "i = " << i << ", " << "mappedInputBytes[i] = "  << mappedInputBytes[i] << std::endl;
#endif
	}
...
#ifdef PING_DEBUG3
      std::cout << "MultiMaskPartialKeyMapping<1u>::extractMaskForMappedInput" << std::endl;
      std::cout << "mappedInputData[0] = " << std::bitset<64>(mappedInputData[0]) << std::endl;
      std::cout << "mExtractionData[0] = " << std::bitset<64>(mExtractionData[0]) << std::endl;
#endif
...
template<unsigned int numberExtractionMasks> inline uint32_t MultiMaskPartialKeyMapping<numberExtractionMasks>::getMaskFor(DiscriminativeBit const & significantKeyInformation) const {
  std::cout << "getMaskFor function: " << std::endl;
  //  using ExtractionDataArray = std::array<uint64_t, numberExtractionMasks>; usigned int
	ExtractionDataArray extractionData = zeroInitializedArray();
	setExtractionByteAt(
		extractionData,
...
template<unsigned int numberExtractionMasks> inline uint32_t MultiMaskPartialKeyMapping<numberExtractionMasks>::getAllMaskBits() const {
  #ifdef PING_DEBUG3
  printf("getAllMaskBits function() \n");
  #endif
...
template<> inline typename MultiMaskPartialKeyMapping<1u>::ExtractionDataArray MultiMaskPartialKeyMapping<1u>::getUsedExtractionBitsForMask(uint32_t usedMaskBits) const {
  #ifdef PING_DEBUG3
  std::cout << "MultiMaskPartialKeyMapping_getUsedExtractionBitsForMask() function:" << std::endl;
  #endif
//  printf("usedMaskBits = %u, mExtractionData[0] = %lld\n", usedMaskBits, mExtractionData[0]);
  /*
   * _pdep_u64(uint_64 a, uint_64 mask): deposit contiguous low bits from input a to dst at the bit position specified by mask
   */
...
template<unsigned int numberExtractionMasks> template<typename PartialKeyType> inline PartialKeyType MultiMaskPartialKeyMapping<numberExtractionMasks>::getMostSignifikantMaskBit(PartialKeyType mask) const {
  #ifdef PING_DEBUG3
  std::cout << "MultiMaskPartialKeyMapping_getMostSignifikantMaskBit() function:" << std::endl;
  #endif
