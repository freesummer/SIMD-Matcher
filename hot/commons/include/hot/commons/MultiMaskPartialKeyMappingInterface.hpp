...
#include "hot/commons/include/hot/commons/SIMDHelper.hpp"
...
private:
  ...
  uint16_t mNumberExtractionBytes;  //  related to numberExtractionMasks: each one = 64-bit masks
...
public:
  ...
  inline uint32_t extractMask(uint8_t const *keyBytes, uint64_t mask) const;
