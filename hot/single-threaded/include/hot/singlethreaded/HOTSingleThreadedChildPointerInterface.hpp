...
/*
 * For HOTSingleThreadedChildPointer
 * Added a listNode pointer
 * added a listNode size
 * added a priority argument
 */
...
/*
 * hides any previously declared meaning for the name name in the tag name space and declares
 * name as a new struct name in current scope, which will be defined later. Until the definition appears,
 * this struct name has incomplete type.
...
//  std::vector<hot::commons::entryVector> entryList;

  std::array<hot::commons::entryVector, 8> entryList;
  uint8_t listSize;
  ...
  /*
   * added the new function with new mask argument
   */
  inline HOTSingleThreadedChildPointer(intptr_t leafValue, uint64_t mask);

  inline HOTSingleThreadedChildPointer(intptr_t leafValue, uint64_t mask, uint64_t priority, std::array<hot::commons::entryVector, 8> WildcardList);
    inline HOTSingleThreadedChildPointer(intptr_t leafValue, uint64_t mask, uint64_t priority, std::array<hot::commons::entryVector, 8> WildcardList, uint8_t listsize);
  ...
  /**
   * Extracts the mask of this child pointer.
   * Be aware that this is only defined if this child pointer was initialized as a leaf child pointer.
   * In all other cases the result is undefined.
   *
   * @return the stored mask
   */
