...
template<typename ValueType, template <typename> typename KeyExtractor> struct HOTSingleThreaded {
  ...
  HOTSingleThreadedChildPointer mRoot_left;
  HOTSingleThreadedChildPointer mRoot_right;
  ...
  idx::contenthelpers::OptionalValue <ValueType> extractAndMatchLeafValue( HOTSingleThreadedChildPointer const & current, KeyType const &key, uint64_t mask);
  ...
  inline unsigned int searchForInsert(uint8_t const * keyBytes, std::array<HOTSingleThreadedInsertStackEntry, 64> & insertStack, uint64_t mask);  // added this new function with a new argument "mask"
  ...
  inline bool insert(ValueType & value, uint64_t mask); //  since we need to change the value, so remove the const
  ...
  inline bool insertWithInsertStack(std::array<HOTSingleThreadedInsertStackEntry, 64> &insertStack, unsigned int leafDepth,
                    KeyType const &existingKey, uint8_t const *newKeyBytes, ValueType const &newValue, uint64_t mask);

  inline bool insertWithInsertStack(std::array<HOTSingleThreadedInsertStackEntry, 64> &insertStack, unsigned int leafDepth,
                    KeyType const &existingKey, uint8_t const *newKeyBytes, ValueType const &newValue, uint64_t mask, uint64_t priority, std::array<hot::commons::entryVector, 8> WildcardList);

  inline bool insertWithInsertStack(std::array<HOTSingleThreadedInsertStackEntry, 64> &insertStack, unsigned int leafDepth,
                    KeyType const &existingKey, uint64_t matchedMask, uint8_t const *newKeyBytes, ValueType const &newValue, uint64_t mask, uint64_t priority, std::array<hot::commons::entryVector, 8> WildcardList);
...
public:
  ...
  std::pair<size_t, std::map<std::string, double>> getStatistics_left() const;

  std::pair<size_t, std::map<std::string, double>> getStatistics_right() const;
...
inline void insertNewWildcardRuleIntoNode(
...
inline void insertNewValueIntoNode(
  std::array<HOTSingleThreadedInsertStackEntry, 64> & insertStack, hot::commons::DiscriminativeBit const & significantKeyInformation,
  unsigned int insertDepth, unsigned int leafDepth, HOTSingleThreadedChildPointer const & valueToInsert, uint64_t mask
);
...
inline void integrateBiNodeIntoTree(
  std::array<HOTSingleThreadedInsertStackEntry, 64> & insertStack, unsigned int currentDepth,
  hot::commons::BiNode<HOTSingleThreadedChildPointer> const & splitEntries, bool const newIsRight, uint64_t mask
);
